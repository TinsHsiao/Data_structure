...
class Maze {
	...
	public:
	stack<int> x_G, y_G ; 				// record 'G' point 
	...
	void FindGoal_V( int i, int j, int direction ) { 		// find the path which will be passed by, mission 1
		...
		if ( foundall || target == 0 ) return ;  				// all target has been found 
	...
	void Shortest_R( int i, int j, int direction, vector<vector<char> > & b, int step ) {
		...
		if ( maze[i][j] == 'G' && i == y_G.top() && j == x_G.top() ) {  // find one road to 'G' 
			// store the shortest one to new maze
	...
	void Shortest_V( int i, int j, int direction, int step, vector<vector<bool> > & vis ) {
		...
		if ( maze[i][j] == 'E'  || maze[i][j] == 'G'  ) { 	// because the maze has been changed from 'E' to 'V'
			...
			if ( direction == 0 ) { // right 
				...
				if ( Judge( i, j + 1 ) ) Shortest_V( i, j + 1, 0, step + 1, vis ) ;
	
				if( Judge( i + 1, j ) )  Shortest_V( i + 1, j, 1, step + 1, vis ) ;

				if ( Judge( i - 1, j ) ) Shortest_V( i - 1, j, 3, step + 1, vis ) ;
			...
			if ( direction == 1 ) {
				...
				if ( Judge( i, j - 1 ) ) Shortest_V( i, j - 1, 2, step + 1, vis ) ;
	...
	void Command3( int maze_G ) {
		maze_G = TotalG() ; // this maze have how many 'G'		
		vector<vector<char> > storeMaze ; // to store the 
		while ( maze_G -- ) {
			
			Shortest_R( 0, 0, 0, storeMaze, 1 ) ; // find which 'G' will appear the shortest path, and store the shortest path 
			x_G.pop() ;
			y_G.pop() ;
			
		} // while 
		
		int tempshort = shortstep ;
		shortstep = 0 ;
		// BFS() ;
		vector<vector<bool > >  vis( row, vector<bool> ( column, false ) ) ;
		Shortest_V( 0, 0, 0, 1, vis ) ; 	 
		for ( int i = 0 ; i < row ; i ++ ) {
			for ( int j = 0 ; j < column ; j ++ ) {
				if ( vis[i][j] ) maze[i][j] = 'V' ;
				cout << maze[i][j] ;
			}
		...
		if ( ! storeMaze.empty() ) {
			...
			cout << endl ;
			cout << "Shortest path length = " << tempshort << " " << shortstep << endl ;
		}
