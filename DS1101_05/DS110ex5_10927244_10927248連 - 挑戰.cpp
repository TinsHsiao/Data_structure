...
TreeList deleteG( TreeList &head, int test ) {
	if ( head != NULL ) {
		if ( head->data.nGrad > test ) {
            head->right = deleteG( head->right, test ) ;
            head->left = deleteG( head->left, test ) ;
		}

		else if ( head->data.nGrad <= test ) {

            if ( head != NULL ) {
                TreeList tmp = head->right ;
                if ( head->right != NULL ) {
...
/*void deleteG( TreeList &head, int test ) {
    if ( head != NULL ) {
        if ( head->data.nGrad > test ) {
            deleteG( head->left, test ) ;
            deleteG( head->right, test ) ;
        }
            ...
            delete head->left ;
            head->left = NULL ;
            if ( head->right != NULL ) deleteG( head->right, test ) ;
            else if ( head  NULL ) preOrderPrint( head ) ;
        }
    }
} */
